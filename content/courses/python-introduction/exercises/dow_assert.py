...
for test_id,date in enumerate(testdays):
    day,month,year=date
    doW=DoW(day,month,year)
    answer=days[answerdays[test_id]]
    assert doW==answer, f"For test {test_id} expected result {answer}, got {doW}"
...
while month < 13:
    doW=DoW(day,month,year)
    answer=days[answerdays[counter]]
    assert doW==answer, f"For test {test_id} expected result {answer}, got {doW}"
    month += 1
    counter += 1
    test_id += 1
