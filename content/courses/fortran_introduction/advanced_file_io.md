...
**Project**
Using the [cpi.csv](/data/cpi.csv) data from a previous project, write a program that will read from the command line the name of a file. Read this file into your program.  Request from the user a year (use non-advancing IO). Once again, do not assume you know in advance how many lines the file contains. Check that you have enough command line input. Stop with a message if you don’t have enough command line values. Use the inquire statement to check that the file exists before you attempt to open it. You can add a message to stop, e.g. stop “Invalid file”.  Test that the file can be opened as part of your open statement.
