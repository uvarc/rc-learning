  ...
  ! Compute steady-state solution
   do while ( iterations<=maxiter )
     ...
     ! Send left and right
       ...
       call MPI_Irecv(w(nrl+1,0) ,1,row,down,tag, grid_comm,mpi_requests(3))
       call MPI_Irecv(w(0,0)     ,1,row,up,tag, grid_comm,mpi_requests(4))
       ...
       call MPI_Isend(w(nrl,0)  ,1,row,up,tag, grid_comm,mpi_requests(7))
       call MPI_Isend(w(1,0),    1,row,down,tag, grid_comm,mpi_requests(8))
