...
int main (int argc, char *argv[]) {
  ...
  while ( iterations<=MAX_ITER ) {
     ...
     MPI_Irecv(&w[0][1], ncl, MPI_DOUBLE, up, tag, MPI_COMM_WORLD, &requests[4]);
     MPI_Irecv(&w[nrl+1][1], ncl, MPI_DOUBLE, down, tag, MPI_COMM_WORLD, &requests[5]);
     MPI_Irecv(&w[0][0], 1, col, left, tag, MPI_COMM_WORLD, &requests[6]);
     MPI_Irecv(&w[0][ncl+1], 1, col, right, tag, MPI_COMM_WORLD, &requests[7]);


     MPI_Isend(&w[1][1], ncl, MPI_DOUBLE, up, tag, MPI_COMM_WORLD, &requests[0]);

     MPI_Isend(&w[nrl][1],ncl, MPI_DOUBLE, down, tag, MPI_COMM_WORLD, &requests[1]);
     MPI_Isend(&w[0][ncl], 1, col, right, tag, MPI_COMM_WORLD, &requests[2]);
     MPI_Isend(&w[0][1], 1, col, left, tag, MPI_COMM_WORLD, &requests[3]);
