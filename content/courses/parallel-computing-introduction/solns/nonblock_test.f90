...
!         print *,grid_rank, grid_coords, u(nrl+1,:)
  ...
  ! Compute steady-state solution
   ...
   call MPI_Irecv(w(0,0)     ,1,row,up,tag, grid_comm,mpi_requests(3))
   call MPI_Irecv(w(nrl+1,0) ,1,row,down,tag, grid_comm,mpi_requests(4))
   ...
   call MPI_Isend(w(nrl,0)  ,1,row,down,tag, grid_comm,mpi_requests(8))
   call MPI_Isend(w(1,0),    1,row,up,tag, grid_comm,mpi_requests(7))
