program sendrows
   ...
   integer, dimension(2) :: dims, ldims, gdims, grid_coords, start_arr
...
!         print *,grid_rank, grid_coords, u(nrl+1,:)
  ...
  ! Write solution to output file by row.
   ...
   print *, grid_rank, start_arr

   call MPI_TYPE_CREATE_SUBARRAY(ndims, gdims, ldims, start_arr,               &
                                MPI_ORDER_FORTRAN, MPI_DOUBLE_PRECISION, locarr)
   call MPI_TYPE_COMMIT(locarr)

   amode=ior(MPI_MODE_CREATE, MPI_MODE_WRONLY)

   fname=trim(filename)
   call MPI_FILE_OPEN(grid_comm, fname, amode, MPI_INFO_NULL, fh, mpi_err)
