...
* `w` (or `-`), indicates write permission, that is, the permission (or absence of permission) to change a file
* `x` (or `-`), indicates execution permission, that is, the permission (or absence of permission) to execute a file, where appropriate
...
```
This will leave the other permissions unaffected. To give read and write permissions on the file biglist to all, type
```
% chmod a+rw biglist
```
...
| Command or Character | Operation |
|---|---|
