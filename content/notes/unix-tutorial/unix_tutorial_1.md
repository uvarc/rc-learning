...
In Unix, folders are generally called _directories_.  Directories are arranged in a hierarchical structure, like an inverted tree. The top of the hierarchy is traditionally called the *root*.
...
The command `cd directory` means change the current working directory to "directory". The current working directory may be thought of as the directory you are in, i.e. your current position in the directory tree. To change to the directory you have just made, type
```
% cd unixstuff
```
...
% ls -a
```
The unixstuff directory (and in all other directories) contains two special directories called period (.) and double period (..). In Unix, period (.) means the current directory, so typing
```
% cd .
```
means stay where you are (the unixstuff directory). This may not seem very useful at first, but using period (.) as the name of the current directory will save a lot of typing, as we shall see later in the tutorial. Double period (..) means the parent of the current directory, so typing
```
% cd ..
```
...
% pwd
```
Most of the time you should see `/home/mst3k`.  On a multiuser central system like Rivanna, /home may be a _symbolic link_, i.e. an alias, for something else.  To see what that link actually points to, type
```bash
% pwd -P
```
...
A relative path starts from the current working directory.  The special symbols period (`.`) and double period (`..`) are often used for relative paths.    
